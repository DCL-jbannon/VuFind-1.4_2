...
CREATE TABLE IF NOT EXISTS `analytics_page_view` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `sessionId` int(11) DEFAULT NULL,
  `pageStartTime` int(11) DEFAULT NULL,
  `pageEndTime` int(11) DEFAULT NULL,
  `module` varchar(128) DEFAULT NULL,
  `action` varchar(128) DEFAULT NULL,
  `method` varchar(128) DEFAULT NULL,
  `objectId` varchar(128) DEFAULT NULL,
  `fullUrl` varchar(2048) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `analytics_search` (
  ...
  `lookfor` varchar(256) DEFAULT NULL,
  `isAdvanced` tinyint(4) DEFAULT NULL,
  `facetsApplied` tinyint(4) DEFAULT NULL,
  `numResults` int(11) DEFAULT NULL,
  `searchTime` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `sessionId` (`sessionId`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `analytics_session` (
  ...
  `homeLocationId` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `bad_words` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='Stores information about bad_words that should be removed fr';
...
CREATE TABLE IF NOT EXISTS `book_store` (
  ...
  `image` varchar(256) DEFAULT NULL COMMENT 'The URL to the icon/image to display',
  `resultRegEx` varchar(100) NOT NULL COMMENT 'The regex used to check the search results',
  `showByDefault` tinyint(4) NOT NULL DEFAULT '1' COMMENT 'Whether or not the book store should be used by default for al library systems.',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `epub_files` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='E-Pub files that can be viewed within VuFind.';
...
CREATE TABLE IF NOT EXISTS `marc_import` (
  ...
  `checksum` bigint(20) NOT NULL COMMENT 'The checksum of the id when it was last imported.',
...
) ENGINE=MyISAM DEFAULT CHARSET=latin1 COMMENT='Caches information from Millennium so we do not have to cont';
...
CREATE TABLE IF NOT EXISTS `nearby_book_store` (
  ...
  PRIMARY KEY (`id`),
  KEY `libraryId` (`libraryId`,`storeId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `notifications_sent` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `type` enum('returnEcontent') NOT NULL,
  `uniqueIdentifier` varchar(254) NOT NULL,
  `userId` int(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `obituary` (
  ...
  `dateDay` int(11) DEFAULT NULL COMMENT 'The day of the month the obituary came out empty or null if not known',
  `dateMonth` int(11) DEFAULT NULL COMMENT 'The month the obituary came out, null or blank if not known',
  `dateYear` int(11) DEFAULT NULL COMMENT 'The year the obituary came out, null or blank if not known',
...
CREATE TABLE IF NOT EXISTS `resource_callnumber` (
  ...
  `callnumber` varchar(50) CHARACTER SET latin1 NOT NULL DEFAULT '',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `roles` (
  ...
  PRIMARY KEY (`roleId`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='A role identifying what the user can do.  A user can have multiple roles assigned to them.';
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `notificationReview` tinyint(4) NOT NULL DEFAULT '1',
  PRIMARY KEY (`id`),
...
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 COMMENT='The epub reading history for patrons';
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='The reading history for patrons';
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Links users with roles so users can perform administration functionatlity';
...
ALTER TABLE `notifications_sent` ADD `notes` VARCHAR( 254 ) NOT NULL AFTER `userId`;

CREATE TABLE IF NOT EXISTS `notifications_mail_statistics` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `type` enum('open','click','rate','review') NOT NULL,
  `uniqueIdentifier` varchar(254) NOT NULL,
  `timestamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `api_clients` (
  ...
  `clientAuthCode` varchar(254) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `api_sessions` (
  ...
  PRIMARY KEY (`id`),
  KEY `clientKeyAccessToken` (`clientId`,`accessToken`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
