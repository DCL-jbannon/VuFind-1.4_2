...
public class AttachEContent implements IProcessHandler {
	...
	public void doCronProcess(String servername, Ini configIni, Section processSettings, Connection vufindConn, Connection econtentConn, CronLogEntry cronEntry, Logger logger) {
		...
		try {
			...
			getRelatedRecords = econtentConn.prepareStatement("SELECT id, accessType, source, availableCopies FROM econtent_record WHERE isbn like ? AND accessType = 'acs'");
	...
	protected boolean addFileToAcsServer(String type, File sourceFile, ImportResult result, String availableCopies){
		...
		try {
			...
			if (responseData.has("error")){
				...
				{
					processLog.addNote(" - Unable to attach the eContent");
					return true;
				}
