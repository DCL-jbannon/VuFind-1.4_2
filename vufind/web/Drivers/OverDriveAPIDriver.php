...
//        return $overDriveDriver2->updateLendingOptions();
    ...
    //TODO repoint at getDownloadLinks()
    ...
    public function getDownloadLinks($overDriveId, $user){
        global $configArray;
        global $analytics;
        ...
        $response = $this->_callPatronUrl($user->cat_username, null, $url);
        //print_r($response);

        $result = array();

        if (isset($response->formats)){
            foreach($response->formats as $format) {

                if($format->formatType == "ebook-overdrive") {
                    $result[] = array(
                        "formatType"=>$format->formatType,
                        "url"=>$format->linkTemplates->downloadLink->href,
                        "text"=>"OverDrive Read");
                } else {
                    $linkResp = $this->_callPatronUrl($user->cat_username, null,
                        str_replace("{errorpageurl}", urlencode($configArray['Site']['url'] . '/Help/OverDriveError'), $format->linkTemplates->downloadLink->href));
                    $result[] = array(
                        "formatType"=>$format->formatType,
                        "url"=>$linkResp->links->contentlink->href,
                        "text"=>"Access eContent");
                }
            }
            if ($analytics) $analytics->addEvent('OverDrive', 'Get Download Link', 'succeeded');
        }else{
