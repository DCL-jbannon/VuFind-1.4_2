...
<div id ="searchStars{if $summShortId}{$summShortId}{else}{$summId|escape}{/if}" class="resultActions">
  <div class="rate{if $summShortId}{$summShortId}{else}{$summId|escape}{/if} stat">
    ...
    <div class="addToBookBag">
      <a id="add_to_cart_{$summId|escape}" class="button add_to_cart" href="#" onclick="toggleInBag('{$summId|escape:"url"}', '{$summTitle|regex_replace:"/(\/|:'\")$/":""|escape:"javascript"}', this); return false;"><span class="add_to_cart">{translate text='Add to cart'}</span><span class="in_cart" style="display:none;">{translate text='Remove from cart'}</span></a>
    </div>
  </div>
  <script type="text/javascript">
    $(
