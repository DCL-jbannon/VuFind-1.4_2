...
{if $user}
	{if count($checkedOut) > 0}
		<div data-role="collapsible-set" data-theme="a" data-content-theme="c">
								...
								<ul data-role="listview" data-theme="a" data-inset="true">
	            					...
	            					{foreach from=$record.links item=link}
										...
										{if !$returnLink}
											<li>
												<a href="{if $link.url}
															{if $isIOSPortalDeviceCompatible && $isIDCLReader}
																idclreader://fulfill/{$link.url}
															{else}
																idclreader://fulfill/{$link.url}
															{/if}
														{else}#{/if}" target='_blank' rel="external">
			...
			{/foreach}
			{if $i eq 0}
				You have no eContent checked out
			{/if}
	...
	{else}
	    	<p>You have no eContent checked out</p>
	{/if}
{else}
