 ...
 *
 */
require_once dirname(__FILE__).'/IndexEngine.php';
require_once dirname(__FILE__).'/Proxy_Request.php';
require_once dirname(__FILE__).'/ConfigArray.php';
require_once dirname(__FILE__).'/SolrUtils.php';
require_once dirname(__FILE__).'/VuFindCache.php';
...
class Solr implements IndexEngine {
	...
	private function _update($xml)
	{
		...
		if ($responseCode == 500 || $responseCode == 400 || $responseCode == 503) {
	...
	public function tokenizeInput($input)
	{
		...
		for ($i=0; $i<count($words); $i++) {
			if (($words[$i] == 'OR') || ($words[$i] == 'AND') || ($words[$i] == 'NOT')) {
				...
				$tmpWord = preg_replace('/[\p{P}]/u', '', $words[$i]);
				if (strlen($tmpWord) > 0){
					$newWords[] = $tmpWord;
				}
			}
