	...
	function GetSuggestions(){
		...
		foreach ($suggestions as $suggestion){
			$titles[] = array(
	    		...
	    		'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=". $suggestion['titleInfo']['id'] . "&isn=" . $suggestion['titleInfo']['isbn10'] . "&size=medium&upc=" . $suggestion['titleInfo']['upc'] . "&category=" . $suggestion['titleInfo']['format_category'][0]."&t=".DateTimeUtils::getTSBookCover(),
	...
	function GetListTitles(){
		...
		foreach ($listTitles as $title){
			if ($title->source == 'VuFind'){
				...
				$titles[] = array(
		    		'id' => $title->record_id,
		    		'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $title->record_id . "&isn=" . $title->isbn . "&size=large&upc=" . $upc . "&category=" . $formatCategory."&t=".DateTimeUtils::getTSBookCover(),
		    		'title' => $title->title,
		    		'author' => $title->author,
				    'source' => 'VuFind',
				...
				if ($record->find(true)){
					$titles[] = array(
						'id' => $record->id,
						'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $record->id . "&isn=" . $record->getIsbn() . "&size=large&upc=" . $record->upc . "&category=EMedia&t=".DateTimeUtils::getTSBookCover(),
						'title' => $record->title,
