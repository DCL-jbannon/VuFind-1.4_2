...
class ListAPI extends Action {
	...
	function getListTitles($listId = NULL, Pagination $pagination = NULL) {
		...
		elseif (preg_match('/review:(.*)/', $listId, $reviewInfo)){
			...
			while ($comments->fetch())
			{
				...
				$resource->source = NULL; //Right now can be VuFind or eContent
				$resource->find(true);
				
				if($resource->format_category=='emedia')
				{
					$recordIds['emedia'][$resource->record_id] = 'econtentRecord'.$resource->record_id;
					$reviews['econtentRecord'.$resource->record_id] = $comment;
					$datesSaved['econtentRecord'.$resource->record_id] = $comments->created;
				}
				else
				{
					$recordIds['print'][$resource->record_id] = $resource->record_id;
			...
			$titles = array();
			if (isset($recordIds['print']))
			{
				$titles = $this->loadTitleInformationForIds($recordIds['print'], $reviews, $datesSaved);
				if(!empty($titles))
				{
					foreach ($titles as $key=>$val)
					{
						$titles[$key]['recordtype'] = 'printTitle'; //econtentRecord for eContent!!!
					}
				}
			}
			
			if (isset($recordIds['emedia']))
			{
				$titlesEcontent = $this->loadEcontentTitleInformationForIds($recordIds['emedia'], $reviews, $datesSaved);
				if(!empty($titlesEcontent))
				{
					foreach ($titlesEcontent as $key=>$val)
					{
						$titlesEcontent[$key]['recordtype'] = 'econtentRecord'; //econtentRecord for eContent!!!
					}
				}
				$titles = array_merge($titles, $titlesEcontent);
			}
			...
			if ($listId == 'newfic'){
				...
				$eContentRecord = new EContentRecord;
				if ($listId == 'newebooks') {
					$eContentRecord->whereAdd("source IN ('3M', 'OverDrive', 'ACS')");
				}
			...
			elseif ($listId == 'newACSOverDriveItems')
			{
				SwitchDatabase::switchToEcontent();
				require_once ('sys/eContent/EContentRecord.php');
				$eContentRecord = new EContentRecord;
				$eContentRecord->whereAdd("accessType='acs'");
				$eContentRecord->whereAdd("source = 'OverDrive'", "OR");
				$eContentRecord->orderBy('date_added DESC');
				$eContentRecord->limit(0,30);
				$eContentRecord->find();
			...
			elseif($listId == 'HighesRatedACSOverDrive')
			{
				SwitchDatabase::switchToEcontent();
				require_once ('sys/eContent/EContentRecord.php');
				$sql="SELECT er.id as id
					  FROM econtent_record as er join (
					  									SELECT recordId, AVG( rating ) AS avgrat, MAX( dateRated ) AS lastDate
														FROM econtent_rating
														GROUP BY `recordId`
														ORDER BY avgrat DESC , `lastDate` DESC
					  								   ) as rat on er.id=rat.recordId
					  WHERE accessType='acs'
					  OR source = 'OverDrive'
					  ORDER BY rat.avgrat DESC, rat.lastDate DESC
					  LIMIT 0,30;";
				$result = mysql_query($sql);
				$titles = $this->eContentRecordMysqlResultToEContentRecord($result);
				SwitchDatabase::restoreDatabase();
				return array('success'=>true, 'listTitle' => $systemList['title'], 'listDescription' => $systemList['description'], 'titles'=>$titles, 'cacheLength'=>1);
			}
			elseif($listId == 'MostPopularACS')
			{
				SwitchDatabase::switchToEcontent();
				require_once ('sys/eContent/EContentRecord.php');
				$sql="SELECT er.id as id
					  FROM econtent_record as er join (
														SELECT recordId, COUNT(recordId) as totalCheckOut, MAX(`dateCheckedOut`) as lastCheckedOut
														FROM econtent_checkout
														GROUP BY recordId
														ORDER BY totalCheckOut DESC, lastCheckedOut DESC
													  ) as co on er.id=co.recordId
					  WHERE accessType='acs'
					  OR source = 'OverDrive'
					  ORDER BY co.totalCheckOut DESC, co.lastCheckedOut DESC
					  LIMIT 0,30;";
				$result = mysql_query($sql);
				$titles = $this->eContentRecordMysqlResultToEContentRecord($result);
				SwitchDatabase::restoreDatabase();
			...
			elseif ($listId == 'recentlyReviewed'){
				...
				foreach ($suggestions as $id=>$suggestion){
					$titles[] = array(
            'id' => $id,
            'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $id . "&isn=" . $suggestion['titleInfo']['isbn10'] . "&size=medium&upc=" . $suggestion['titleInfo']['upc'] . "&category=" . $suggestion['titleInfo']['format_category'][0]."&t=".DateTimeUtils::getTSBookCover(),
	...
	private function eContentRecordMysqlResultToEContentRecord($result)
	{
		$titles = array();
		if($result)
		{
	...
	private function setEcontentRecordInfoForList($eContentRecord)
	{
		...
		return array(
				...
				'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $eContentRecord->id . "&isn=" . $eContentRecord->getIsbn() . "&size=medium&upc=" . $eContentRecord->getUpc() . "&category=EMedia&econtent=true&t=".DateTimeUtils::getTSBookCover(),
				'large_image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $eContentRecord->id . "&isn=" . $eContentRecord->getIsbn() . "&size=large&upc=" . $eContentRecord->getUpc() . "&category=EMedia&econtent=true&t=".DateTimeUtils::getTSBookCover(),
				'small_image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $eContentRecord->id . "&isn=" . $eContentRecord->getIsbn() . "&size=small&upc=" . $eContentRecord->getUpc() . "&category=EMedia&econtent=true&t=".DateTimeUtils::getTSBookCover(),
	...
	function loadEcontentTitleInformationForIds($ids, $descriptions = array(), $datesSaved = array())
	{
		require_once ('sys/eContent/EContentRecord.php');
		global $configArray;
		$titles = array();
		if (count($ids) > 0)
		{
			//This method is only for biblio, so make sure that uses always this shard
			$tpmShardSession = $_SESSION['shards'];
			unset($_SESSION['shards']);
			$_SESSION['shards'][1] = "eContent";
			...
			foreach ($matchingRecords as $eContentRecord)
			{
				$id = str_replace("econtentRecord", "", $eContentRecord['id']);
				
				$econtentDBRecord = new EContentRecord();
				$econtentDBRecord->id = $id;
				if($econtentDBRecord->find())
				{
					$econtentDBRecord->fetch();
					$isbn = $econtentDBRecord->getissn();
					
					$description = $descriptions[$eContentRecord['id']];
					...
					$titles[] = array
								(
						            'id' => $id,
						            'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $id . "&isn=" . $isbn . "&size=medium&econtent=true&t=".DateTimeUtils::getTSBookCover(),
						            'large_image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $id . "&isn=" . $isbn . "&size=large&econtent=true&t=".DateTimeUtils::getTSBookCover(),
									'small_image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $id . "&isn=" . $isbn . "&size=small&econtent=true&t=".DateTimeUtils::getTSBookCover(),						
						            'title' => $eContentRecord['title'],
									...
									'dateSaved' => isset($datesSaved[$eContentRecord['id']]) ? $datesSaved[$eContentRecord['id']] : '',
								);
				}
			}
		}
	...
	function loadTitleInformationForIds($ids, $descriptions = array(), $datesSaved = array())
	{
		...
		$tpmShardSession = $_SESSION['shards'];
		unset($_SESSION['shards']);
		$_SESSION['shards'][0] = "Main Catalog";
		...
		if (count($ids) > 0)
		{
			...
			foreach ($matchingRecords as $record)
			{
				...
				$titles[] = array
							(
						            'id' => $record['id'],
						            'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $record['id'] . "&isn=" . $isbn . "&size=medium&upc=" . (isset($record['upc']) ? $record['upc'][0] : '') . "&category=" . $format_category."&t=".DateTimeUtils::getTSBookCover(),
						            'large_image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $record['id'] . "&isn=" . $isbn . "&size=large&upc=" . (isset($record['upc']) ? $record['upc'][0] : '') . "&category=" . $format_category."&t=".DateTimeUtils::getTSBookCover(),
									'small_image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $record['id'] . "&isn=" . $isbn . "&size=small&upc=" . (isset($record['upc']) ? $record['upc'][0] : '') . "&category=" . $format_category."&t=".DateTimeUtils::getTSBookCover(),						
						            'title' => $record['title'],
						            'author' => isset($record['author']) ? $record['author'] : '',
	...
	function getRandomSystemListTitles($listName){
		...
		if ($listTitles == false || isset($_REQUEST['reload'])){
			...
			foreach ($matchingRecords as $record){
				...
				$listTitles[] = array(
	          'id' => $record['id'],
	          'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $record['id'] . "&isn=" . $isbn . "&size=medium&upc=" . (isset($record['upc']) ? $record['upc'][0] : '') . "&category=" . $record['format_category'][0]."&t=".DateTimeUtils::getTSBookCover(),
	          'title' => $record['title'],
	          'author' => isset($record['author']) ? $record['author'] : '',
	...
	function getSystemListTitles($listName){
		...
		if ($listTitles == false){
			...
			foreach ($matchingRecords as $record){
				...
				$listTitles[] = array(
	          'id' => $record['id'],
	          'image' => $configArray['Site']['coverUrl'] . "/bookcover.php?id=" . $record['id'] . "&isn=" . $isbn . "&size=medium&upc=" . $record['upc'][0] . "&category=" . $record['format_category'][0]."&t=".DateTimeUtils::getTSBookCover(),
	          'title' => $record['title'],
