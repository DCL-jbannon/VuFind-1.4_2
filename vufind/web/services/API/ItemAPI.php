...
class ItemAPI extends Action
{
	...
	function getItem()
	{
		...
		if ($record['recordtype'] == 'econtentRecord'){
			...
			if (!$eContentRecord->find(true)){
				...
				$itemData['cover'] = $configArray['Site']['coverUrl'] . "/bookcover.php?id={$itemData['id']}&isbn={$itemData['isbn']}&upc={$itemData['upc']}&category={$itemData['formatCategory']}&format={$itemData['format'][0]}&size=medium&t=".DateTimeUtils::getTSBookCover();
	...
	function getBasicItemInfo(){
		...
		if ($record['recordtype'] == 'econtentRecord'){
			...
			$itemData['formatCategory'] = $record['format_category'][0];
			$itemData['language'] = $record['language'];
			$itemData['cover'] = $configArray['Site']['url'] . "/bookcover.php?id={$itemData['id']}&isbn={$itemData['isbn']}&upc={$itemData['upc']}&category={$itemData['formatCategory']}&format={$itemData['format'][0]}&t=".DateTimeUtils::getTSBookCover();
