...
class Record extends Action
{
	...
	function __construct($subAction = false, $record_id = null)
	{
		...
		$bookCoverUrl = $configArray['Site']['coverUrl'] . "/bookcover.php?id={$this->id}&amp;isn={$this->isbn}&amp;size=large&amp;upc={$this->upc}&amp;category=" . urlencode($format_category) . "&amp;format=" . urlencode(isset($recordFormat[0]) ? $recordFormat[0] : '')."&t=".DateTimeUtils::getTSBookCover();
