...
class DBMaintenanceEContent extends Admin {
	...
	private function getSQLUpdates() {
      ...
      'initial_setup' => array(
		...
		'createActiveEcontentRecordsTable' => array(
			'title' => 'Add active_econtent_records table',
			'description' => 'Add table to track econtent records that are still in MARC file',
			'dependencies' => array(),
			'sql' => array('CREATE TABLE active_econtent_records(ilsId INTEGER, PRIMARY KEY(ilsId));'),
		),
		'alterEcontentAttacheTable' => array(
				'title' => 'Alter econtent_attach table',
