...
function makeAjaxCall(GetStatusList)
{
	//console.log("BEFORE AJAX CALL: " + GetStatusList);
	requestNumber++;
	if(requestNumber<GetStatusList.length)
	{
			var now = new Date();
			...
			url += "&id[]=" + encodeURIComponent(GetStatusList[requestNumber]);
			url += "&time="+ts;
			$.getJSON(url, function(data)
			{
				makeAjaxCall(GetStatusList);
			...
			success: function(data){
				...
				$(items).each(function(index, item){
					...
					if ($(item).find('showplacehold').text() == 1){
						...
						var formatType = $(item).find('formatType').text();
						switch(formatType)
						{
							case 'eBook':
								$("#access-eBook" + elemId).show();
								break;
							case 'eAudio':
								$("#access-eAudio" + elemId).show();
								break;
							case 'eVideo':
								$("#access-eVideo" + elemId).show();
								break;
							case 'eMusic':
								$("#access-eMusic" + elemId).show();
								break;
							default:
		...
		function(data, textStatus) {
			...
			for (var id in recordRatings){
				...
				if (recordRatings[id].user != null && recordRatings[id].user > 0){
					...
					$('.rate' + id).each(function(index)
										 {
											$(this).rater({'rating':data['standard'][id].average, 'doBindings':false, module:'Record', recordId: id});
										 }
					);
				}
			...
			for (var id in eContentRatings){
				// Load the rating for the title
				if (eContentRatings[id].average != null && eContentRatings[id].average > 0){
					$('.rateEContent' + id).each(function(index){
						$(this).rater({'rating':eContentRatings[id].average, 'doBindings':false, module:'EcontentRecord', recordId: id });
					});
				}else{
