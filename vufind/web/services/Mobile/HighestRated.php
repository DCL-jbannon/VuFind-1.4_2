...
class HighestRated extends Action {
	...
	function launch()
	{
		...
		$listTitlesHR = $memcache->get('HighesRatedACSOverDrive');
		if($listTitlesHR ===  false )
		{
			$listAPI = new ListAPI();
			$listTitlesHR = $listAPI->getListTitles('HighesRatedACSOverDrive');//Check success key
			$memcache->set("HighesRatedACSOverDrive", $listTitlesHR, MEMCACHE_COMPRESSED, 3600);
		}
		
		$interface->assign('LIST',($listTitlesHR['success'] ? $listTitlesHR['titles'] : ""));
		$interface->caching = 0;
		$cacheId = 'IdclReader - Highest Rated|' . $interface->lang;
