...
class MostPopular extends Action {
	...
	function launch()
	{
		...
		$listTitlesMP = $memcache->get('MostPopularACS');
		if($listTitlesMP ===  false )
		{
			//Most Popular
			$listAPI = new ListAPI();
			$listTitlesMP = $listAPI->getListTitles('MostPopularACS');//Check success key
			$memcache->set("MostPopularACS", $listTitlesMP, MEMCACHE_COMPRESSED, 3600);
		}
